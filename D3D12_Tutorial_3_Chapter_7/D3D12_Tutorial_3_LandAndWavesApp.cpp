...
void LandAndWavesApp::Draw(const GameTimer& gt)
{
	...
	// https://stackoverflow.com/questions/65444256/error-c2102-requires-l-value-vs2019-how-to-fix
	auto c = CurrentBackBufferView();
	auto d = DepthStencilView();
	mCommandList->OMSetRenderTargets(1, &c, true, &d);
...
void LandAndWavesApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	auto xmm_d_v = XMMatrixDeterminant(view);
	XMMATRIX invView = XMMatrixInverse(&xmm_d_v, view);
	auto xmm_d_p = XMMatrixDeterminant(proj);
	XMMATRIX invProj = XMMatrixInverse(&xmm_d_p, proj);
	auto xmm_d_vp = XMMatrixDeterminant(viewProj);
	XMMATRIX invViewProj = XMMatrixInverse(&xmm_d_vp, viewProj);
...
void LandAndWavesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	{
		auto ri = ritems[i];
		auto ri_g_v = ri->Geo->VertexBufferView();
		auto ri_g_i = ri->Geo->IndexBufferView();

		cmdList->IASetVertexBuffers(0, 1, &ri_g_v);
		cmdList->IASetIndexBuffer(&ri_g_i);
